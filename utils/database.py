...
def store_embedding(text, metadata, collection_name="agent_learning"):
    try:
        ...
        try:
            ...
            # Fallback for Python versions < 3.9 where usedforsecurity is not available
        ...
        if metadata and metadata.get("source"):
            ...
            if (collection_name, src) not in _known_sources_cache:
                try:
                    ...
                    logger.warning(f"Optimization warning: Failed to update sources index: {e}")
...
def get_available_metadata_sources(collection_name="agent_learning"):
    try:
        ...
        try:
            ...
            logger.debug(f"Failed to query sources collection {source_col_name}: {e}")
        ...
        if unique_sources:
            try:
                ...
                logger.warning(f"Failed to populate sources collection: {e}")
...
def delete_embeddings(collection_name="agent_learning"):
    try:
        ...
        try:
            ...
            logger.debug(f"Failed to delete sources collection (may not exist): {e}")
    except Exception as e:
